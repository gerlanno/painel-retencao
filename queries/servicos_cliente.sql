...
FROM
    ...
    LEFT JOIN tbladdress a ON a.relid = h.id
    AND a.subtype = 'INSTALACAO'
    LEFT JOIN tblhostingnetworkconfig nc ON nc.hosting_id = h.id
    LEFT JOIN tblnetworktechnology t ON t.id = nc.technology_id
WHERE
    h.userid = %s